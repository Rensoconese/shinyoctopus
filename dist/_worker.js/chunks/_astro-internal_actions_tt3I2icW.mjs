...
const contactFormSchema = objectType({
  ...
  message: stringType().min(10, { message: "Message must be at least 10 characters long" }),
  source: stringType(),
  otherSource: stringType().optional()
});
  ...
  async send(formData) {
    ...
    try {
      ...
      const { data: emailData, error } = await resend.emails.send({
        from: "Shiny Octopus <hi@shinyoctopus.studio>",
        // Usando el dominio verificado en Resend
        to: ["hola@shinyoctopus.com.ar"],
...
export { generateFormToken as g, server as s };
